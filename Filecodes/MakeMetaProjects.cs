public static void MakeMetaProjects(string topDir) {
  ...
  foreach (string taskDir in Directory.GetDirectories(topDir)) {
      ...
      if (!Directory.Exists(secretProjectDir))
          throw new Exception("Make sure creating the 
          secret project before the meta project");
      ...
      string secretFileName = secretFile.Substring(secretFile.
      LastIndexOf("\\") + 1);
      ...
      foreach (var studentDir in Directory.GetDirectories(taskDir)) {
        ...
        foreach (var file in Directory.GetFiles(studentDir)) {
          if (file.EndsWith(".cs")) {
            ...
            string projectName = "meta_project" + fileName.Substring(0, 
            fileName.Length - 3);
            ...
            CreateMetaProgram(metaProgramFile, secretFile, 
            secretAssembly, file);
